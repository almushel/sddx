...
void draw_particles(Particle_System* ps, SDL_Renderer* renderer) {
	for (int p = 0; p < ps->particle_count; p++) {
		Particle particle = ps->particles[p];
		if (particle.sprite) {
			// draw sprite
...
Particle* instantiate_particle(Particle_System* ps, Uint32 sprite, Particle_Shape shape) {
...
void explode_at_point(Particle_System* ps, float x, float y, float force, rgb_color* colors, Uint32 num_colors, Uint32 sprite, Particle_Shape shape) {
